...
error_font = pygame.font.SysFont("Comic Sans MS", 20)
instructions_font = pygame.font.SysFont("Comic Sans MS", 24)
...
while not done:
    ...
    for event in pygame.event.get():  # User did something
        if event.type == pygame.QUIT or event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE:  # Click close or ESC
    ...
    label = instructions_font.render("Use arrowkeys to navigate and space to solve.", 1, (0, 0, 0))
            ...
            if input_sudoku[i][j] != 0:
                if solved:
                    if input_sudoku[i][j] != sudoku.arr[i][j]:
                        label = error_font.render(str(input_sudoku[i][j]), 1, (255, 0, 0))
                        screen.blit(label, (j * square_side + 5, i * square_side))
                        continue
                    else:
                        label = my_font.render(str(input_sudoku[i][j]), 1, (0, 255, 0))
                else:
                    label = my_font.render(str(input_sudoku[i][j]), 1, (150, 150, 150))
                screen.blit(label, (j * square_side + square_side * 0.3, i * square_side))
    if not solved:
        pygame.draw.rect(screen, (255, 0, 0), pygame.Rect(col * square_side, row * square_side, square_side, square_side),
                     2)
